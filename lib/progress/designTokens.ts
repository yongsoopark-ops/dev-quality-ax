...
/** 서브 세부 목표 · 공통 업무 항목 공용 마커 — 텍스트 글리프(○/●)를 CSS
 * 원으로 교체했다. ○와 ●는 폰트마다 글리프 자체의 실제 지름이 달라서
 * (font-size를 맞춰도 렌더 크기가 어긋남) 11×11px 원으로 그리고 예정만 흰
 * 배경(테두리만), 진행중·완료는 테두리와 같은 색으로 채운다 — 세 상태의
 * 실제 렌더 width/height가 완전히 같아야 한다(getBoundingClientRect로
 * 검증). 공통 업무 항목은 WAITING/DONE 2상태만 쓴다(IN_PROGRESS 미해당,
 * 월별 계획은 한 달 안에서 진행중 구분이 큰 의미가 없다는 판단) — 마커는
 * 서브와 완전히 동일한 규격을 그대로 재사용한다.
 *  - WAITING     → Tailwind neutral-300(#d4d4d4) 테두리 + 흰 배경
 *  - IN_PROGRESS → Tailwind amber-700(#b45309) 테두리+배경(기존 보류/개선
 *    차수 배지와 동일 토큰, 새 hex를 만들지 않았다) — 서브 전용
 *  - DONE        → Tailwind green-700(#15803d) 테두리+배경(완료=초록 의미를
 *    정규/서브/공통 전부 동일하게 유지) */
